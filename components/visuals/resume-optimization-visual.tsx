...
export default function ResumeOptimizationVisual({
  ...
  const highlightBox = useMemo(() => {
    ...
    const lastLine = contentLines[highlightedIndices[highlightedIndices.length - 1]];
    return {
      y: firstLine.y - 8,
  ...
  const HIGHLIGHT_DELAY = LINE_START_DELAY + (contentLines.length * LINE_STAGGER) + 0.2;
    ...
    <div ref={ref} className={`w-full h-full flex items-center justify-center ${className}`}>
      ...
      >
        ...
        <motion.rect 
          ...
          transition={{ duration: 0.4, delay: LINE_START_DELAY + LINE_STAGGER }}
        />
