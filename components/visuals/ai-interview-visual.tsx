  ...
  return (
    <div ref={ref} className={`w-full h-full flex items-center justify-center ${className}`}>
      ...
      >
        ...
        {/* Header Area */}
        <motion.path
          d={`M ${CARD_X} 36 A 16 16 0 0 1 ${CARD_X + 16} 20 H ${CARD_X + CARD_WIDTH - 16} A 16 16 0 0 1 ${CARD_X + CARD_WIDTH} 36 V 70 H ${CARD_X} V 36 Z`}
          fill="#F9FAFB"
          initial={{ opacity: 0 }}
        ...
        <motion.circle
          ...
          transition={{ duration: 0.4, delay: HEADER_DELAY + 0.1, type: "spring" }}
        />
        <motion.text
          x={CARD_X + 30}
          y="49"
        ...
        <motion.text
          ...
          animate={isInView ? { opacity: 1, x: CARD_X + 55 } : { opacity: 0, x: CARD_X + 45 }}
        ...
        {qLines.map((line, i) => (
          <motion.text
            ...
            initial={{ opacity: 0 }}
            animate={isInView ? { opacity: 1 } : { opacity: 0 }}
            transition={{ duration: 0.3, delay: QUESTION_DELAY + 0.2 + (i * 0.1) }}
          >
            {line}
          </motion.text>
        ))}
        ...
        {aLines.map((line, i) => (
          <motion.text
            ...
            initial={{ opacity: 0 }}
            animate={isInView ? { opacity: 1 } : { opacity: 0 }}
            transition={{ duration: 0.3, delay: ANSWER_DELAY + 0.2 + (i * 0.1) }}
          >
            {line}
          </motion.text>
        ))}
        ...
        <motion.g
          ...
          transition={{ duration: 0.4, delay: FEEDBACK_DELAY, type: "spring" }}
        >
          <circle cx={RIGHT_BUBBLE_X + BUBBLE_WIDTH - 20} cy={aY + (aHeight/2)} r="16" fill="#FFF3E0" stroke="#FFB74D" strokeWidth="1" />
          <circle cx={RIGHT_BUBBLE_X + BUBBLE_WIDTH - 20} cy={aY + (aHeight/2)} r="12" fill="#FEB271" fillOpacity="0.2" />
          <motion.path
            d={`M ${RIGHT_BUBBLE_X + BUBBLE_WIDTH - 26} ${aY + (aHeight/2)} L ${RIGHT_BUBBLE_X + BUBBLE_WIDTH - 22} ${aY + (aHeight/2) + 4} L ${RIGHT_BUBBLE_X + BUBBLE_WIDTH - 14} ${aY + (aHeight/2) - 4}`}
            stroke="#FEB271"
            strokeWidth="2.5"
            strokeLinecap="round"
